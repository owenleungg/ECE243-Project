...
void apply_grey_scale() {
    ...
    for (int i = 0; i < PADDING_WIDTH * PADDING_HEIGHT; i++)
    {
        uint16_t pixel = input_frame[i];
        ...
        // Calculate grayscale
        uint16_t gray = (uint16_t)(0.3 * r + 0.59 * g + 0.11 * b);

        // Convert 8-bit gray back to RGB565
        uint16_t gray_r5 = (uint16_t)((gray / 255.0) * 31 + 0.5);  // Round to nearest
        uint16_t gray_g6 = (uint16_t)((gray / 255.0) * 63 + 0.5);
        uint16_t gray_b5 = (uint16_t)((gray / 255.0) * 31 + 0.5);
...
void apply_gaussian_kernal() {
    ...
    for (int x = kernal_radius; x < SCREEN_WIDTH - kernal_radius; x++) {
        for (int y = kernal_radius; y < SCREEN_HEIGHT - kernal_radius; y++) {
            double accum = 0.0;
            ...
            for (int ky = -kernal_radius; ky <= kernal_radius; ky++) {
                for (int kx = -kernal_radius; kx <= kernal_radius; kx++) {
                    ...
                    uint16_t pixel = grayscale[(img_y << 9) + img_x];
            ...
            uint16_t final_intensity = (uint16_t)accum;
            uint16_t r5 = (final_intensity * 31) / 255;
            uint16_t g6 = (final_intensity * 63) / 255;
            uint16_t b5 = (final_intensity * 31) / 255;

            blurred[(y << 9) + x] = (r5 << 11) | (g6 << 5) | b5;
        }
    }
}
        ...
        for (int y = 1; y < SCREEN_HEIGHT - 1; y++) {
            ...
            for (int ky = -1; ky <= 1; ky++) {
                for (int kx = -1; kx <= 1; kx++) {
                    ...
                    uint16_t pixel = blurred[(img_y << 9) + img_x];
            ...
            uint16_t r5 = (gradient_mag * 31) / 255;
            uint16_t g6 = (gradient_mag * 63) / 255;
            uint16_t b5 = (gradient_mag * 31) / 255;
            gradient[(y << 9) + x] = (r5 << 11) | (g6 << 5) | b5;
        }
    }
}
...
void apply_non_max_suppression() {
    for (int x = 1; x < SCREEN_WIDTH - 1; x++) {
        for (int y = 1; y < SCREEN_HEIGHT - 1; y++) {
            int gx = gx_buffer[(y << 9) + x];
            int gy = gy_buffer[(y << 9) + x];
    
            float angle = atan2(gy, gx) * 180.0 / M_PI;
            if (angle < 0) angle += 180;
    
            int current = gradient[(y << 9) + x];
            ...
            if ((angle >= 0 && angle < 22.5) || (angle >= 157.5 && angle <= 180)) {
                ...
                neighbor1 = gradient[(y << 9) + (x + 1)];
                neighbor2 = gradient[(y << 9) + (x - 1)];
            } else if (angle >= 22.5 && angle < 67.5) {
                // Northeast and southwest neighbors
                neighbor1 = gradient[((y + 1) << 9) + (x - 1)];
                neighbor2 = gradient[((y - 1) << 9) + (x + 1)];
            } else if (angle >= 67.5 && angle < 112.5) {
                // North and south neighbors
                neighbor1 = gradient[((y + 1) << 9) + x];
                neighbor2 = gradient[((y - 1) << 9) + x];
    ...
    for (int x = 1; x < SCREEN_WIDTH - 1; x++) {
        for (int y = 1; y < SCREEN_HEIGHT - 1; y++) {
            ...
            if (intensity > HIGH_THRESHOLD) {
                thresholded[(y << 9) + x] = gradient[(y << 9) + x];
                thresholds[(y << 9) + x] = STRONG_EDGE;
            } else if (intensity > LOW_THRESHOLD) {
                thresholded[(y << 9) + x] = gradient[(y << 9) + x];
                thresholds[(y << 9) + x] = WEAK_EDGE;
            } else {
                thresholded[(y << 9) + x] = 0;
                thresholds[(y << 9) + x] = 0;
            }
...
void apply_edge_tracking() {
    for (int x = 1; x < SCREEN_WIDTH - 1; x++) {
        for (int y = 1; y < SCREEN_HEIGHT - 1; y++) {
            ...
            if (thresholds[(y << 9) + x] == STRONG_EDGE) {
                // Strong edge
                hysteresis[(y << 9) + x] = 0xF800;
                continue;
            }

            if (thresholds[(y << 9) + x] == 0) {
                // Not Strong or weak edge
                hysteresis[(y << 9) + x] = 0;
                continue;
            }

            if (thresholds[(y << 9) + x] == WEAK_EDGE) {
                // Weak edge
                ...
                for (int dy = -1; dy <= 1; dy++) {
                    for (int dx = -1; dx <= 1; dx++) {
                        ...
                        if (thresholds[((y + dy) << 9) + (x + dx)] == STRONG_EDGE) {
